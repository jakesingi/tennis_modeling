functions {
  ...
  real negbin_lpdf(real[] y, vector p, vector r) {
    return sum(r.*log(p) + to_vector(y).*log1m(p));
  }
}
data {
...
model {
  ...
  winner_y_hard ~ negbin(get_set_win_prob(inv_logit(X_skill_hard*(alpha_vector[1:J]) + (X1_hard - X2_hard)*beta)), r_hard);
  winner_y_clay ~ negbin(get_set_win_prob(inv_logit(X_skill_clay*(append_row(alpha_vector[1:J], alpha_vector[(J+1):(2*J)])) + (X1_clay - X2_clay)*beta)), r_clay);
  winner_y_grass ~ negbin(get_set_win_prob(inv_logit(X_skill_grass*(append_row(alpha_vector[1:J], alpha_vector[(2*J+1):(3*J)])) + (X1_grass - X2_grass)*beta)), r_grass);
}
