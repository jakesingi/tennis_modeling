functions {
  ...
  // Computes the log-PDF of the negative binomial distribution with k failures until r successes
...
model {
  ...
  winner_y_hard ~ negbin(pwinner_hard_set, r_hard);
  winner_y_clay ~ negbin(pwinner_clay_set, r_clay);
  winner_y_grass ~ negbin(pwinner_grass_set, r_grass);
}
