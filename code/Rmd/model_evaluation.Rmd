...
```{r}
# Multi-T models
multi_t_games_test_out = readRDS("./multi_t_models2/multi_t_games_test_frames.RData") %>%
  rename(winner_y = w_games, n = games) %>%
  mutate(model = "Multi-T_bin_games",
         p1_won_previous_set = NA,
         p2_won_previous_set = NA) %>%
  select(-best_of)
multi_t_bernoulli_sets_test_out = do.call(rbind, readRDS("./multi_t_models3/bernoulli_sets_test_frames.RData")) %>%
  rename(winner_y = y) %>%
  mutate(model = "Multi-T_bernoulli_sets",
         n = NA,
         pp_p1_y = NA) %>%
  select(-best_of)
missing1_multi_t_negbin_sets_test_out = readRDS("./multi_t_models2/multi_t_negbin_sets_test_frames.RData") %>%
  rename(winner_y = sets_won_loser, n = best_of) %>%
  mutate(model = "Multi-T_negbin_sets",
         p1_won_previous_set = NA,
         p2_won_previous_set = NA)

missing2_multi_t_negbin_sets_test_out = do.call(rbind, readRDS("./multi_t_models2/rerun_multi_t_negbin_sets_test_frames.RData")) %>%
  rename(winner_y = sets_won_loser, n = best_of) %>%
  mutate(model = "Multi-T_negbin_sets",
         p1_won_previous_set = NA,
         p2_won_previous_set = NA)
...
# Multi-T
multi_t_games_post_mean_dfs = readRDS("./multi_t_models2/multi_t_games_post_mean_dfs.RData")
multi_t_games_post_mean_dfs = multi_t_games_post_mean_dfs[lengths(multi_t_games_post_mean_dfs) != 0]
multi_t_bernoulli_sets_post_mean_dfs = readRDS("./multi_t_models3/bernoulli_sets_post_mean_dfs.RData")
multi_t_negbin_sets_post_mean_dfs = readRDS("./multi_t_models2/multi_t_negbin_sets_post_mean_dfs.RData")
multi_t_negbin_sets_post_mean_dfs = multi_t_negbin_sets_post_mean_dfs[lengths(multi_t_negbin_sets_post_mean_dfs) != 0]
# Multi-Norm
